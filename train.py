...
def main():
    ...
    if args.train_type == "lora":
        model = MODE[args.mode]["model"].from_pretrained(args.model_name_or_path,device_map={"": args.local_rank}, trust_remote_code=True,torch_dtype=torch.float16)
        ...
        config = MODE[args.mode]["config"].from_pretrained(args.model_name_or_path,device_map={"": args.local_rank}, trust_remote_code=True)
        config.pre_seq_len = args.pre_seq_len
        config.prefix_projection = args.prefix_projection
        model = MODE[args.mode]["model"].from_pretrained(args.model_name_or_path, config=config,device_map={"": args.local_rank}, trust_remote_code=True,torch_dtype=torch.float16)
        for name, param in model.named_parameters():
